document.writeln("</br> </br> Javascript </br>");

//Membuat Variable Menggunakan var 
        // Menggunakan Deklarasi Variable
        var keyboard1;
        var huruf1;
        var nomor1;

            keyboard1 = "ABCD 1234"
            huruf1 = "ABCD";
            nomor1 = "1234";

        document.writeln(keyboard1 + "</br>");
        document.writeln(huruf1 + "</br>");
        document.writeln(nomor1);

        // Langsung Menggunakan Value
        var keyboard2 = "ABCDE 12345";
        var huruf2 = "ABCDE";
        var nomor2 = "12345";

        document.writeln(keyboard2 + "</br>");
        document.writeln(huruf2 + "</br>")
        document.writeln(nomor2)

    //Membuat Variable Menggunakan let dan const   
        // let
        let keyboard3 = "ABCDEF 123456";
        let huruf3 = "ABCDEF";
        let nomor3 = "123456";

        document.writeln(keyboard3 + "</br>")
        document.writeln(huruf3 + "</br>")
        document.writeln(nomor3)

        // const
        const madeWith = "Javascript";

        document.writeln(madeWith);